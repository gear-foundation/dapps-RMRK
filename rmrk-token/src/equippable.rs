...
impl Assets {
    pub fn add_equippable_asset_entry(
        ...
        if let Some(address) = catalog_address {
            ...
            self.part_ids.insert(id, part_ids.clone());
        }
        self._add_asset_entry(id, metadata_uri.clone())?;
    ...
    fn add_asset_entry(&mut self, metadata_uri: String) -> Result<RMRKReply, RMRKError> {
        let id = (self.assets.len() as u64) + 1;
    ...
    pub fn accept_asset(
        ...
        Ok(RMRKReply::AssetAccepted { token_id, asset_id })
    }
    ...
    pub fn equip(
        &mut self,
        state: TxState,
        _tx_data: Option<Vec<u8>>,
        args: (TokenId, TokenId, ActorId, u64, PartId, u64),
    ) -> Result<(TxState, MessageId), RMRKError> {
        let (token_id, child_token_id, child_id, asset_id, slot_part_id, child_asset_id) = args;
        let catalog_address = self.catalog_addresses.get(&asset_id).expect("");
        match state {
            TxState::Initial => {
                // check whether the slot is used
                if let Some(parts) = self.equipments.get(&(token_id, *catalog_address)) {
                    if let Some(_) = parts.get(&slot_part_id) {
                ...
                if let Some(part_ids) = self.part_ids.get(&asset_id) {
                    if part_ids.iter().any(|&part_id| part_id == slot_part_id) {
                        ...
                        return Ok((TxState::MsgCheckEquippableSent, msg_id));
                    }
                }
                return Err(RMRKError::TargetAssetCannotReceiveSlot);
            }
            TxState::ReplyCheckEquippableReceived => {
                ...
                return Ok((TxState::MsgCanTokenBeEquippedSent, msg_id));
            }
...
pub fn equip_reply(tx: &mut Tx, processing_msg_id: MessageId) {
    let state = tx.state.clone();
    match state {
        TxState::MsgCheckEquippableSent => {
            let reply: Result<CatalogReply, CatalogError> =
                msg::load().expect("Failed to decode the reply");
            match reply {
                Ok(CatalogReply::InEquippableList) => {
                    tx.state = TxState::ReplyCheckEquippableReceived;
                }
                Ok(CatalogReply::NotInEquippableList) => {
                    tx.state = TxState::Error(RMRKError::NotInEquippableList);
                }
                Err(CatalogError::WrongPartFormat) => {
                    tx.state = TxState::Error(RMRKError::WrongPartFormat);
                },
                _ => {
                    tx.state = TxState::Error(RMRKError::UnknownError);
                }
            }
            exec::wake(processing_msg_id).expect("Failed to wake the message");
        }
        TxState::MsgCanTokenBeEquippedSent => {
            let reply: Result<RMRKReply, RMRKError> =
                msg::load().expect("Failed to decode the reply");
            match reply {
                Ok(RMRKReply::TokenBeEquippedWithAssetIntoSlot) => {
                    tx.state = TxState::ReplyCanTokenBeEquippedReceived;
                },
                Err(error) => {
